class Control {
  ...
  get element() {
    return this.#parent.querySelector(
      `.controls__arrow_${this.to > 0 ? "next" : "prev"}`
    );
  }
...
export class Slider {
  ...
  set #currentSlide(nextSlide) {
    ...
    if (this.#infinite) {
      ...
      if (this.#controls) {
        ...
        const control =
          this.#direction > 0 ? this.#controls.next : this.#controls.prev;

        isInRange ? control.enable() : control.disable();

        const oppositeControl =
          this.#direction > 0 ? this.#controls.prev : this.#controls.next;
        if (isInRange) {
    ...
    if (this.#numerated) {
      this.#root.querySelector(".controls__from").textContent =
        this.#currentSlide;
    }

    if (this.#dots) {
      this.#getDot(previousSlide).classList.remove(this.#ACTIVE_DOT_CLASSNAME);
      this.#getDot(this.#currentSlide).classList.add(
        this.#ACTIVE_DOT_CLASSNAME
      );
    }
  }
  ...
  #replaceSlides(to) {
    ...
    const replaceSlides = () => {
      this.setTranslate(
        (to > 0 ? 1 : this.#totalSlides) + this.#getAdaptiveAdjustGap(),
        true,
        false
      );
  ...
  #initIndicators() {
    ...
    if (this.#totalSlides && this.#numerated) {
      this.#root.querySelector(".controls__to").textContent = `/ ${
        this.#totalSlides
      }`;
    }
  ...
  #getAdaptiveAdjustGap() {
    const slidesPerPage =
      this.#slidesPerPage ?? this.#getSlidesPerPagePropertyValue();
    ...
    window.addEventListener("resize", () => {
      ...
      if (this.#mobileOnly) {
        if(window.innerWidth > this.#breakpoints.mobile) {
          this.setTranslate(0, true, false);
        } else {
          this.setTranslate(this.#currentSlide, true, false);
        }
      }
    });
  }
  ...
  setTranslate(to, isIn, withAnimation = true) {
    ...
    if (this.#spaceBetween) {
      translateValue = `calc(${translateValue} + ${
        this.#spaceBetween * (this.#slideProgress / 100)
      }px)`;
    }
  ...
  register({
    ...
    this.#breakpoints = {
      tablet: breakpoints?.tablet ?? 1023,
    ...
    initialSlide &&
      this.setTranslate(
        initialSlide + this.#getAdaptiveAdjustGap(),
        true,
        false
      );
